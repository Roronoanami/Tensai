...
//     CommunityStatusResponse response = new CommunityStatusResponse();

//     response.setOwner(isOwner);
//     response.setMember(member.isPresent());

//     if (member.isPresent()) {
//         response.setRole(member.get().getRole());
//     }
...
// boolean isOwner = community.getOwnerId()
//         .equals(user.getId());


// // OWNER CAN JOIN WITHOUT PASSWORD
// if (!isOwner && community.isPrivateCommunity()) {
...
public CommunityStatusResponse getCommunityStatus(
    ...
    System.out.println("Owner ID = " + community.getOwnerId());
